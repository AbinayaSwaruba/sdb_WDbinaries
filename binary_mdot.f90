...
! ***********************************************************************
      ...
      subroutine set_accretion_composition(b, acc_index)
         ...
         real(dp) ::h1_xa,he3_xa

         integer :: k, nz, h1,he4,he3,j
         
         h1 = b%s_accretor% net_iso(ih1)
         he4 = b%s_accretor% net_iso(ihe4)
         he3  = b%s_accretor% net_iso(ihe3)
         ...
         if (acc_index == b% a_i) then
           ...
           do j = 1, b% s_donor% species
               if (b% s_accretor% accretion_species_id(j) == b% s_accretor% accretion_species_id(h1)) then
                  b% s_accretor% accretion_species_xa(j) = 0
                  h1_xa = b% s_donor% xa_removed(j)
               else if (b% s_accretor% accretion_species_id(j) == b% s_accretor% accretion_species_id(he3)) then
                  b% s_accretor% accretion_species_xa(j) = 0
                  he3_xa = b% s_donor% xa_removed(j) 
               else if (b% s_accretor% accretion_species_id(j) == b% s_accretor% accretion_species_id(he4)) then
                  b% s_accretor% accretion_species_xa(j) = b% s_donor% xa_removed(j) + h1_xa + he3_xa
               else
                  b% s_accretor% accretion_species_id(j) = chem_isos% name(b% s_donor% chem_id(j))
            ...
            if (b% s_accretor% star_mdot > 0) then
               do j = 1, b% s_donor% species
                  write (*,*) b% s_accretor% accretion_species_id(j),b% s_accretor% accretion_species_xa(j) 
               end do
            end if
