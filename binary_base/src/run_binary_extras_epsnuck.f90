...
! *********************************************************************** 
      ...
      integer function extras_binary_check_model(binary_id)
         ...
         nz = b% s2% nz
         h1 = b% s2% net_iso(ih1)
         he4 = b% s2% net_iso(ihe4)
         do k=1,nz
            if (b% s2% xa(h1, k) > b% s2% xa(he4, k)) then
               b% s2% eps_nuc(k) = 0
               !write(*, *) 'eps nuc set to zero at cell number with h and he',k,b% s2% xa(h1, k),b% s2% xa(he4, k),nz,b% s2% eps_nuc(400),b% s2% eps_nuc(550)
            end if 
         end do
